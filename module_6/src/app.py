...
def _ensure_limit(sql: str) -> str:
    """
    Ensure a SQL string includes a LIMIT clause.

    If the query already contains LIMIT (case-insensitive), it is returned unchanged.
    Otherwise, a conservative "LIMIT 1" is appended.

    Args:
        sql: SQL query string.

    Returns:
        SQL query string guaranteed to contain a LIMIT clause.
    """
    normalized = sql_utils.normalize_sql(sql)
    return normalized if sql_utils.has_limit(normalized) else f"{normalized} LIMIT 1"
...
def fetch_one(
    ...
    Normalizes whitespace and guarantees a LIMIT clause (adds LIMIT 1 if missing).
...
def build_blocks() -> list[dict[str, Any]]:
    ...
    blocks.append(
        {
            "title": (
                "Q1) How many entries do you have in your database that have applied for "
                "Fall 2026?"
            ),
            "headers": h,
            "rows": r,
    ...
    blocks.append(
        {
            "title": (
                "Q3) What is the average GPA, GRE, GRE V, and GRE AW of applicants who "
    ...
    blocks.append(
        {
            "title": (
                "Q6) What is the average GPA of applicants who applied for Fall 2026 and "
                "were accepted?"
            ),
            "headers": h,
            "rows": r,
    ...
    blocks.append(
        {
            "title": (
                "Q7) How many entries are from applicants who applied to JHU for a masters "
    ...
    blocks.append(
        {
            "title": (
                "Q8) How many entries from 2026 are acceptances from applicants who "
                "applied to Georgetown University, MIT, Stanford University, or Carnegie "
                "Mellon University for a PhD in Computer Science?"
            ),
            "headers": h,
            "rows": r,
        }
    ...
    blocks.append(
        {
            "title": (
                "Q9) Do your numbers for question 8 change if you use LLM-generated "
...
def run_pull_pipeline() -> None:  # pragma: no cover
    """
    Execute scraping, cleaning, and loading pipeline.

    Prevents concurrent execution using a lock file.
    Uses the current Python interpreter to avoid Windows launcher/PATH issues.
    """
    try:
        create_lock()
        ...
        if scrape.exists():
            subprocess.run([sys.executable, str(scrape)], check=True)

        if clean.exists():
            subprocess.run([sys.executable, str(clean)], check=True)

        subprocess.run([sys.executable, str(APP_DIR / "load_data.py")], check=True)
