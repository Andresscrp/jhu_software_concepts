...
def test_clean_module_main_guard_runs(tmp_path, monkeypatch):
    ...
    by executing the module as a script.
    """
    monkeypatch.chdir(tmp_path)

    # These are the default filenames used by src.clean in your project.
    (tmp_path / "applicant_data.json").write_text(
        json.dumps([{"program": "Math"}]),
        encoding="utf-8",
    )

    # Run module as script WITHOUT importing src.clean first.
